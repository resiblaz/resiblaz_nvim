...
function M.core_mappings(maps)
  local astrocore = require "astrocore"
  if not maps then maps = astrocore.empty_map_table() end
...
function M.buffer_mappings(maps)
  ...
  maps.n["<C-L>"] = {
    function() utils_u.nav(vim.v.count > 0 and vim.v.count or 1) end,
  ...
  maps.n["<C-W>"] = { function() require("astrocore.buffer").close(0, true) end, desc = "Force close buffer" }
...
--- toggleterm mappings
function M.toggleterm_mappings(maps) maps.t["<Esc>"] = { "<C-\\><C-n>" } end

-- local base_mapsrequire("astronvim")
